...
// DESKSHARE ZERO (CLEAN BASE)
const LOG_FILE = path.join(os.tmpdir(), 'deskshare_zero.log');
...
ipcMain.on('log-stream', (e, msg) => {
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send('log-stream', msg);
    log(`[ENGINE] ${msg}`);
});
...
function createMainWindow() {
    mainWindow = new BrowserWindow({ width: 450, height: 750, show: false, webPreferences: { nodeIntegration: true, contextIsolation: false }, autoHideMenuBar: true, backgroundColor: '#050507', title: "DeskShare Zero" });
...
function createEngineWindow() {
    engineWindow = new BrowserWindow({
        ...
        show: true, // MUST BE VISIBLE FOR PERMISSIONS
        webPreferences: { nodeIntegration: true, contextIsolation: false, backgroundThrottling: false },
        title: "DeskShare Engine [Listening]",
