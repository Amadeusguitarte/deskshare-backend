...
class ChatManager {
    ...
    // ==========================================
    // ZERO-BASE PAIRING (The Magic Link)
    // ==========================================
    pairLauncher() {
        try {
            const token = localStorage.getItem('authToken');
            if (!token) return this.showToast('No hay sesión activa', 'error');

            // Hardcoded to ID 14 for the User's Personal PC as per "Zero-Base" debug requirement.
            ...
            console.error(e);
            this.showToast('Error al iniciar sincronización', 'error');
        }
    }
