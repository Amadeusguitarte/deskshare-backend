...
class ChatManager {
    constructor(currentUser, socketUrl) {
        ...
        // New State for files and full page
        ...
        try {
            ...
            for (const conv of rawConvs) {
                if (!seenIds.has(conv.otherUser.id)) {
                    ...
                    // Preserve existing messages in memory to prevent flashing/wiping
    ...
    startTitleBlink(userName) {
        ...
        this.titleInterval = setInterval(() => {
            document.title = isOriginal ? `💬 Nuevo mensaje de ${userName}` : "DeskShare - Alquila Computadoras Potentes";
    ...
    emitTyping(receiverId) {
        if (this.typingTimeouts[receiverId]) clearTimeout(this.typingTimeouts[receiverId]);
        else this.socket.emit('typing', { receiverId });
    ...
    openLightbox(url, userId) {
        // Simple lightbox for now, can be expanded
        let lightbox = document.getElementById('chat-lightbox');
        if (lightbox) lightbox.remove();

        lightbox = document.createElement('div');
        lightbox.id = 'chat-lightbox';
        lightbox.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 10001; display: flex; align-items: center; justify-content: center; cursor: pointer;';
        lightbox.innerHTML = `<img src="${url}" style="max-width: 90%; max-height: 90%; border-radius: 4px; box-shadow: 0 0 20px rgba(0,0,0,0.5);">`;
    ...
    // ===========================================
    // View Logic - Full Page (RESTORED MODERN DESIGN)
    // ===========================================
    renderFullPage() {
        ...
        // Init Full Page Handlers
        this.initFullPageHandlers();
    }

    initFullPageHandlers() {
        // Helper to separate init logic
    ...
    // ===========================================
    // View Logic - Global Widget (BACKUP V180 STRUCTURE)
    // ===========================================
    renderWidget() {
        if (this.messagesPageContainer) return;
                ...
                <div class="chat-list-area" style="flex: 1; overflow-y: auto; background: #111;">
                    ${this.conversations.length > 0 ? this.conversations.map(conv => `
                        <div id="widget-list-item-${conv.otherUser.id}" onclick="chatManager.openChat(${conv.otherUser.id})" style="padding: 10px; border-bottom: 1px solid #333; cursor: pointer; display: flex; align-items: center; gap: 10px; transition: background 0.2s;" onmouseover="this.style.background='#222'" onmouseout="this.style.background='transparent'">
                            <img src="${conv.otherUser.avatarUrl || 'assets/default-avatar.svg'}" style="width: 32px; height: 32px; border-radius: 50%; object-fit: cover;">
                            <div style="flex:1; overflow:hidden;">
                                ...
                                <div style="font-size: 0.8rem; color: #888; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${(conv.lastMessage?.message || ((conv.lastMessage?.fileUrl) ? '📎 Archivo' : ''))}</div>
                            </div>
    ...
    renderChatTab(conv) {
        ...
        return `
            ...
            // Image Handling
            if (m.fileUrl) {
                if (m.fileType === 'image') content = `<img src="${m.fileUrl}" onclick="window.chatManagerInstance.openLightbox('${m.fileUrl}', '${m.senderId}')" style="max-width: 100%; border-radius: 8px; cursor: zoom-in;">`;
                else content = `<a href="${m.fileUrl}" target="_blank" style="color:white; text-decoration:underline;">📄 Archivo</a>`;
            }

            return `<div style="text-align: ${isMe ? 'right' : 'left'}; margin-bottom: 5px;">
                                    <span style="background: ${isMe ? 'var(--accent-purple)' : '#333'}; padding: 5px 10px; border-radius: 10px; color: white; display: inline-block; max-width: 85%; word-wrap: break-word;">${content}</span>
                                 </div>`;
        }).join('')}
                </div>
                ...
                <div class="chat-footer" style="padding: 12px; border-top: 1px solid #333; background: #222; display: flex; align-items: center; gap: 8px;">
                     <!-- Attach Icon -->
                    <button onclick="chatManager.triggerFileUpload(${user.id})" style="background: none; border: none; cursor: pointer; color: #888; padding: 4px; display: flex; align-items: center;">
                    ...
                    <input type="text" placeholder="Escribe..." 
                           ...
                           style="flex: 1; padding: 8px; border: 1px solid #444; border-radius: 20px; outline: none; font-size: 0.9rem; background: #333; color: white;">
                           
                    <!-- Emoji Placeholder (Optional) -->
                </div>
            </div>
        `;
    }
    ...
    toggleMinimize(userId) {
        ...
        if (tab) tab.style.height = this.minimizedConversations.has(userId) ? '50px' : '400px';
    }
    ...
    handleInputFocus(userId) {
        if (!this.minimizedConversations.has(userId)) {
            ...
            // Stop animation
            const tab = document.getElementById(`chat-tab-${userId}`);
            if (tab) tab.classList.remove('flash-animation');
            this.stopTitleBlink();
        }
    }
