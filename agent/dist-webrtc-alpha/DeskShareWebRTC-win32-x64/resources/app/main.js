...
function loadData() {
    try {
        ...
        if (fs.existsSync(configPath)) {
            ...
            log(`Config Loaded: ${config.computerId}`);
        }
...
ipcMain.handle('api-request', async (e, { method, endpoint, body }) => await nodeRequest(method, endpoint, body));

ipcMain.on('renderer-ready', (event) => { if (config) try { event.reply('init-config', { config, res }); } catch (e) { } });
ipcMain.on('engine-ready', (event) => { if (config) safeSend(engineWindow, 'init-engine', { config, res }); });
ipcMain.on('session-update', (e, sid) => log(`Session: ${sid}`));
...
function writeToInput(cmd) { if (inputProcess) inputProcess.stdin.write(cmd + "\n"); }
...
function createEngineWindow() {
    ...
    async function poll(){
        ...
        try {
            ...
            if (res && res.sessionId) {
                // v21: ALWAYS CHECK OFFER if session exists
                sid = res.sessionId;
                ipcRenderer.send('session-update', sid);
                
                const p = await api('GET', "/webrtc/poll/" + sid);
                if (p && p.offer) {
                    // Only process if we need to (not connected or new offer)
                    if (!pc || pc.connectionState === 'closed' || pc.connectionState === 'failed' || pc.signalingState === 'stable') {
                        handle(p.offer);
                    }
                }
            }
    ...
    async function handle(sdp){
        if(pc && pc.connectionState === 'connected') return; // Don't interrupt good connection
        if(pc) pc.close();
