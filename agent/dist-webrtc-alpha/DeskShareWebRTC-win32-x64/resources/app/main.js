...
log('==========================================');
log('[v20] STARTING AGENT v20...');

// SINGLETON
const gotTheLock = app.requestSingleInstanceLock();
if (!gotTheLock) {
    log('[v20] Duplicate instance detected. Quitting.');
...
let config = null; // Renamed from globalConfig for brevity
let res = { w: 1920, h: 1080 };

process.on('uncaughtException', (err) => log(`[CRITICAL] Uncaught: ${err.message}\n${err.stack}`));

// 1. DATA LOADING
function loadData() {
    log('[Data] Loading config...');
    try {
        const APPDATA = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Preferences' : process.env.HOME + "/.local/share");
        const configPath = path.join(APPDATA, 'deskshare-launcher', 'config.json');
        log(`[Data] Path: ${configPath}`);

        if (fs.existsSync(configPath)) {
            config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
            log(`[Data] Config Loaded for ComputerID: ${config.computerId}`);
        } else {
            log('[Data] ERROR: Config file not found!');
        }

        const primary = screen.getPrimaryDisplay();
        res = { w: primary.size.width, h: primary.size.height };
        log(`[Data] Resolution: ${res.w}x${res.h}`);
    } catch (e) {
        log(`[Data] Load Failed: ${e.message}`);
    }
}

// 2. NETWORK KERNEL (Hardened)
function nodeRequest(method, pathStr, body) {
    return new Promise((resolve) => {
        try {
            log(`[API] Req: ${method} ${pathStr}`);
            if (!config || !config.token) {
                log('[API] ERROR: No token available');
                return resolve(null);
            }
            ...
            const req = https.request(opts, (rs) => {
                ...
                rs.on('data', (c) => d += c);
                rs.on('end', () => {
                    log(`[API] Res: ${rs.statusCode} for ${pathStr}`);
...
// 3. IPC HANDLERS
ipcMain.handle('api-request', async (e, { method, endpoint, body }) => {
    return await nodeRequest(method, endpoint, body);
});

ipcMain.on('renderer-ready', (event) => {
    log('[IPC] Renderer Ready');
    if (config) try { event.reply('init-config', { config, res }); } catch (e) { }
});

ipcMain.on('engine-ready', (event) => {
    log('[IPC] Engine Ready -> Sending Init');
    if (config) safeSend(engineWindow, 'init-engine', { config, res });
    else log('[IPC] WARN: Engine ready but config is missing!');
});

ipcMain.on('session-update', (e, sid) => {
    log(`[Session] Update: ${sid}`);
    // activeSessionId = sid; 
});

ipcMain.on('engine-state', (e, s) => {
    log(`[Engine] State: ${s}`);
    safeSend(mainWindow, 'update-engine-ui', s);
});

// 4. INPUT LOGIC
ipcMain.on('remote-input', (event, data) => {
    // log input only on clicks to save space
    if (data.type !== 'mousemove') log(`[Input] ${data.type}`);
    ...
    if (data.px !== undefined) {
        finalX = Math.round(data.px * res.w);
        finalY = Math.round(data.py * res.h);
    }
...
function startController() {
    try {
        ...
        log(`[Controller] Spawning: ${script}`);
        inputProcess = spawn('powershell.exe', ['-ExecutionPolicy', 'Bypass', '-File', script]);

        inputProcess.stdout.on('data', (d) => log(`[PS1 OUT] ${d}`));
        inputProcess.stderr.on('data', (d) => log(`[PS1 ERR] ${d}`));
...
function createEngineWindow() {
    log('[Window] Creating Engine Window');
...
app.on('will-quit', async () => {
    log('[Lifecycle] Quitting, killing input...');
