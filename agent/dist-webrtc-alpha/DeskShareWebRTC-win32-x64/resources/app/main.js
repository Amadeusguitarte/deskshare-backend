...
// v19: HYBRID ARCHITECTURE (Renderer Logic + Main Networking)
console.log('[v19] Starting DeskShare Agent...');
...
process.on('uncaughtException', (err) => console.error('[v19] CRITICAL:', err));

// 1. NETWORK KERNEL (Main Process)
function nodeRequest(method, path, body, token) {
        ...
        try {
            ...
            const req = https.request(opts, (res) => {
                ...
                res.on('end', () => {
                    ...
                    } else resolve(null); // Silent fail for polling
                });
            });
...
ipcMain.handle('api-request', async (e, { method, endpoint, body }) => {
    ...
    return await nodeRequest(method, endpoint, body, globalConfig.token);
});
