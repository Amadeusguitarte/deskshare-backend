...
const https = require('https'); // v18.1: Native HTTPS (No Axios)

// v18.1: DEPENDENCY-FREE (No node_modules required)
console.log('[v18.1] Starting DeskShare Agent (Standalone)...');
...
process.on('uncaughtException', (err) => console.error('[v18.1] CRITICAL:', err));
...
async function terminateSession() {
    if (activeSessionId && globalConfig && globalConfig.token) {
        // v18.1: Native HTTPS Request
        try {
            const urlStr = `https://deskshare-backend-production.up.railway.app/api/webrtc/session/${activeSessionId}/terminate`;
...
function createEngineWindow() {
    ...
    // v18.1: Native FETCH Engine (No Axios)
    const engineHtml = `<!DOCTYPE html><html><body><script>const {ipcRenderer}=require('electron');let config=null,sid=null,pc=null;ipcRenderer.on('init-engine',(e,d)=>{config=d.config;setInterval(poll,1000)});
    async function api(method,endpoint,body=null){
        const url="https://deskshare-backend-production.up.railway.app/api"+endpoint;
        const opts={method:method,headers:{'Authorization':'Bearer '+config.token,'Content-Type':'application/json'}};
